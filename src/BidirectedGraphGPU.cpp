...
void BidirectedGraphGPU::copy_to_GPU(HandleGraph& host_graph) {
    ...
    neighbor_start = (nid_t*) malloc(2 * size * sizeof(nid_t)); // Placeholder for cudaMalloc
    neighbor_count = (nid_t*) malloc(2 * size * sizeof(nid_t)); // Placeholder for cudaMalloc
    ...
    for (nid_t nid = 1; nid <= size; nid++) {
        ...
        neighbor_count[2 * (nid - 1)] = host_graph.get_degree(handle, false);
        ...
        neighbor_count[2 * (nid - 1) + 1] = host_graph.get_degree(handle, true);
    }
}
